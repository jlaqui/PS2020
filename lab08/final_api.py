...
class Users(Resource):
    ...
    def get(self):
        ...
        if args['userId'] in list(data['userId']):
            arr = list(data['userId']) # parse to list
            index = arr.index(args['userId']) # get index of the list
            data = data.to_dict()  # convert dataframe to dict
            
            return {'userId': data['userId'][index],
                    'name': data['name'][index],
                    'city': data['city'][index]
                    }, 200  # return data[index] and 200 OK
        else:
...
class Locations(Resource):
    def get(self):
        ...
        if args['locationId'] in list(data['locationId']):
            arr = list(data['locationId']) # parse to list
            index = arr.index(args['locationId']) # get index of the list
            data = data.to_dict()  # convert dataframe to dict
            
            return {'locationId': data['locationId'][index],
                    'name': data['name'][index],
